...
const handleSocketConnection = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("subscribeToUsers", (userIds) => {
      console.log("subscribeToUsers", userIds);
    ...
    socket.on("subscribeToConversations", (conversationIds) => {
      console.log("subscribeToConversations", conversationIds);
    ...
    socket.on("SEND_MESSAGE", async ({ conversationId, type, content }) => {
      console.log("SEND_MESSAGE", { conversationId, type, content });
      try {
        const message = {
    ...
    socket.on("TYPING", ({ conversationId, isTyping }) => {
      console.log("TYPING", {conversationId, isTyping});
    ...
    socket.on("MESSAGE_SEEN", async ({ conversationId, messageId }) => {
      console.log("MESSAGE_SEEN", {conversationId, messageId});
    ...
    socket.on("GET_USER_STATUS", async ({ userId }) => {
      console.log("GET_USER_STATUS", {userId});
